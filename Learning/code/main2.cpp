...
#include <tchar.h>
#include <time.h> 
#include "DirectInputClass.h"											//DirectInput类，获取输入信息的
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nShowCmd)
{
	...
	wndClass.cbSize = sizeof( WNDCLASSEX ) ;	//结构体的字节数
	...
	wndClass.lpszClassName = _T("WindowsClassForGAIA");			//指定窗口类的名字。

	if( !RegisterClassEx( &wndClass ) )							//对窗口类进行注册，创建该类型的窗口
		return -1;		

	HWND hwnd = CreateWindow( _T("WindowsClassForGAIA"),WINDOW_TITLE,			//创建窗口函数CreateWindow
	...
	UnregisterClass(_T("WindowsClassForGAIA"), wndClass.hInstance);//注销窗口类
	return 0;  
}
...
bool ObjectsForGui_Init()
{
	...
	// 添加按钮
	if(!g_LocalLoadGUI->AddButton(BUTTON_START_ID, WINDOW_WIDTH - 300, WINDOW_HEIGHT - 70 , L"GameMedia\\startUp.png", L"GameMedia\\startOver.png",
	...
	// 添加按钮
	if(!g_ConnectGUI->AddButton(BUTTON_CONNECT_ID, WINDOW_WIDTH - 300, WINDOW_HEIGHT - 140 , L"GameMedia\\connectUp.png", L"GameMedia\\connectOver.png",
		L"GameMedia\\connectDown.png")) return false;
	if(!g_ConnectGUI->AddButton(BUTTON_BACK_ID, WINDOW_WIDTH - 300, WINDOW_HEIGHT - 70 , L"GameMedia\\backUp.png", L"GameMedia\\backOver.png",
	...
	// 添加按钮
	if(!g_OnlineLoadGUI->AddButton(BUTTON_BACK_ID, WINDOW_WIDTH - 300, WINDOW_HEIGHT - 70 , L"GameMedia\\backUp.png", L"GameMedia\\backOver.png",
		L"GameMedia\\backDown.png")) return false;
	...
	// 添加按钮
	if(!g_OptionGUI->AddButton(BUTTON_BACK_ID, WINDOW_WIDTH - 300, WINDOW_HEIGHT - 70 , L"GameMedia\\backUp.png", L"GameMedia\\backOver.png",
		L"GameMedia\\backDown.png")) return false;
...
//	SetupBoneMatrixPointers(g_pFrameRoot, g_pFrameRoot);//设置好各级框架的组合变换矩阵
...
//	g_pAnimController->SetTrackAnimationSet((UINT)1.0, pAnimationSet);
	...
	// 创建骨骼动画
	g_pAllocateHier = new CAllocateHierarchy();
	D3DXLoadMeshHierarchyFromX(L"tiny_4anim.x", D3DXMESH_MANAGED, g_pd3dDevice, 
		g_pAllocateHier, NULL, &g_pFrameRoot, &g_pAnimController);
	SetupBoneMatrixPointers(g_pFrameRoot, g_pFrameRoot);//设置好各级框架的组合变换矩阵

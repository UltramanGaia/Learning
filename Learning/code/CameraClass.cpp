...
void CameraClass::SetRighthVector(D3DXVECTOR3 *vRight)					//设置右方向向量
{
	m_vRightVector = *vRight;
}


void CameraClass::SetUpVector(D3DXVECTOR3 *vUp)							//设置上方向向量
{
	m_vUpVector = *vUp;
}
void CameraClass::SetLookVector(D3DXVECTOR3 *vLook)						//设置观察方向向量
{
	m_vLookVector = *vLook;
}
...
void CameraClass::FollowPosition(D3DXVECTOR3 *vCharPos)                 // 摄像机跟随某个位置的上帝视角
{
	m_vRightVector		=	D3DXVECTOR3(1.0f, 0.1f, -0.2f);				//默认右向量与X正半轴重合
	m_vUpVector			=	D3DXVECTOR3(0.1f, 0.7f, 0.7f);				//默认上向量与Y正半轴重合
	m_vLookVector		=	D3DXVECTOR3(0.2f, -0.7f, 0.7f);				//默认观察向量与Z正半轴重合
